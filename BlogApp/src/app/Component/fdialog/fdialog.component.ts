...
export class FdialogComponent {
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<FdialogComponent>,
    private route: Router,
    private toastr: ToastrService,
    private dialog: MatDialogRef<FdialogComponent>,
  ...
  getFollowingdata(){
    if (this.id === null || this.id === undefined || this.id === '') {
      ...
      this.api.getFollowing(this.Authtoken, this.id).subscribe({
        next: (res) => {
          this.Following = res.following.Following;
          this.HomeUserData=res.user;
          this.FNewData =res.ff[0].Following;
